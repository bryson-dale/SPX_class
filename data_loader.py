...
class DataLoader:
    ...
    def preprocess_data(self):
        ...
        permno_to_hsiccd = dict(zip(self.permno_industry_map_raw["permno"], self.permno_industry_map_raw["hsiccd"]))
        hsiccd_header = [permno_to_hsiccd.get(int(col), np.nan) for col in self.spx_prices_raw.columns]
        self.spx_prices_raw.columns = pd.MultiIndex.from_arrays([self.spx_prices_raw.columns, hsiccd_header], names=['permno', 'sector'])
    ...
    def get_permno_to_sector_map(self):
        return dict(zip(self.permno_industry_map_raw["permno"], self.permno_industry_map_raw["hsiccd"]))
